    ...
    public class CorePlatform
    {
        ...
        private PlatformState WeaponParts()
        {
            for (int i = 0; i < _orderToCreate.Count; i++)
            {
                ...
                {
                    MyLog.Default.WriteLineAndConsole($"{Comp.SubtypeName}: {system.Values.AutoGenerated} {Parts.NameToEntity.TryGetValue(elevationPartName, out muzzlePartEntity)} ({elevationPartName})");
                    // this is extremely ugly and bad but theres no way to tell if a gatling turret is missing its barrel subpart or not during Init() automatic generation from what I can tell
                    if (system.Values.AutoGenerated && Parts.NameToEntity.TryGetValue(elevationPartName, out muzzlePartEntity))
                    {
                        system.MuzzlePartName = system.ElevationPartName;
                    }
                    else 
